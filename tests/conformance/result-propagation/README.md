...
The operand appears once, as a byte span; its type is the one an unannotated
`let` initializer of the same expression would get, with three exceptions
answered first: `null` is `null`, a parenthesized operand is typed by what it
encloses, and a call to a declared `-> Int?` function is `Int?`.
...
- Thirteen generated sources, one `?` each, cover every expression position:
  ...
  assignment, a lambda body, and a field read after the `?`.
...
- `pair.mjs` runs the Kofun half under the canonical interpreter against the C
  ...
  and scope HIR.
- Four rebuilt mutants: the old path (reader stops at `?` and the nodes are
  ...
  lowering; a skipped stage check falls back to `E2S158`; and a reader that
  stops at `?` mis-spans the lambda body in `grammar.kofun`.
