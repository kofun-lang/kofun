...
| the module is mechanically valid | with its module framing stripped — the single-file profile has no modules or visibility — and concatenated with [`driver.kofun`](driver.kofun), it builds through `kofun build --backend c --c-abi --link-library`, where the independent c_abi compiler re-derives the record layout and `_Static_assert`s it into the emitted C. The stripping is asserted to remove exactly the two header lines and the `pub ` prefixes, and the framing itself is built and run by [`import-boundary/run.sh`](import-boundary/run.sh) |
...
| raw is marked raw | the module declares `trust raw-foreign` as its second line, the report records that class beside the `raw-trusted-foreign` marking and the digest of the exact bytes, and the `.raw.` filename segment, the banner, and the instruction to import only behind a hand-reviewed safe façade are all asserted |
| raw is reachable only through review (#1217) | [`import-boundary/run.sh`](import-boundary/run.sh) builds and runs the three-module chain — generated bindings, reviewed façade, ordinary consumer — and refuses an ordinary import, an aliased import, a `trusted import` of an ordinary module, a raw module with its trust line removed, and **a renamed copy with no `.raw.` segment**, each with no artifact |
