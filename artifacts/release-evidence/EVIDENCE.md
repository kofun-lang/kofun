...
| `benchmark-report-v1` | `task benchmark-report` | The specification, model, codec and comparison gates pass, and one Kofun source path constructs, encodes, decodes and compares complete reports against independent contract expectations at O0/O2, on repeat and under ASan/UBSan. |
...
| `bounded-bytes` | `task bounded-bytes` | The sequential aggregate runs the carrier gate's tracked owner, cleanup, transfer, and refusal fixtures, then the mutation gate's checked-in direct helper, growth, range, transactionality, and E2S177 fixtures, then the file-read gate's digest comparison against bin/kofun-digest and its four registered runtime refusals, then the Text-bridge gate's sanitized round trips, its four registered runtime refusals, its E2S179 value refusal, and its emitted-C tag/detail driver. The observation is fixture-bounded and leaves the specification's known gaps open. |
...
| `positional-move-crossings` | `task move-call-crossings` | Bytes borrowing and one-time transfer, trivial-record read and one-time transfer, and fresh record shadowing execute with exact stdout at O0/O2 under ASan/UBSan. Component boundaries and missing-rule mutations fail by name. |
