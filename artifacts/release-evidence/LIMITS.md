...
| `benchmark-report-v1` | rejection | `tests/stdlib/benchmark-report-codec/fixtures.mjs` | All 44 digest-pinned negative vectors return the frozen code with 48 neutral fields. Every injected encoder/decoder allocation failure returns BR012; prior outcomes including BR011 preserve the exact destination pointer, capacity, length and bytes without exposing a prefix. |
...
| `bounded-bytes` | rejection | `tests/conformance/bytes-mutation/same_carrier.kofun` | A direct unparenthesized builtin append_range call with one named binding in both carrier positions is refused as E2S177, commits no C, and is mutation-proved to commit no binary. |
...
| `macho64-ad-hoc-signing` | rejection | `bootstrap/native/check-macho64-signed.sh` | Unknown CPUs, empty or oversized code, bytes outside 0..255, and empty, oversized, NUL-containing, non-ASCII identifiers refuse before image writing; independent mutations of signed header, code, identifier, page hash, signature magic, and signature offset fail validation. |
...
| `positional-move-crossings` | rejection | `tests/conformance/move-call-crossings/record_edit.kofun` | A trivial-record edit parameter is refused as exact E2S181 at its mode-token span and no C is published; removing the rule reaccepts the defective by-value shape. |
