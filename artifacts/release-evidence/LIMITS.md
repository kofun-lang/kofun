...
| `bounded-bytes` | rejection | `tests/conformance/bytes-mutation/same_carrier.kofun` | A direct builtin append_range call with one named binding in both carrier positions is refused as E2S177, commits no C, and is mutation-proved to commit no binary. |
