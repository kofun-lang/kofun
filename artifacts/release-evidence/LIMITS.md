...
| `bytes-bounded-mutation` | rejection | `tests/diagnostics/stage2/e2s177_bytes_append_range_identity.kofun` | Copying a range between carriers requires two distinct bindings; the same binding on both sides is refused as `E2S177` before any C is emitted. |
