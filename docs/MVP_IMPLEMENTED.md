...
| bounded byte-buffer mutation on the C11 Stage 2 backend | bounded Stage 2/C11 checkpoint: a 65,536-byte carrier with `len`, `capacity`, checked read and write, `clear`, `reserve`, and three append forms, every failure transactional; no `Text` bridge and no atomic replacement of a bound target | `task bytes-mutation` | `bytes-bounded-mutation` |
