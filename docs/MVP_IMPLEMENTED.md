...
| bounded Bytes[65536] carrier and mutation on C11 Stage 2 | bounded C11 Stage 2 checkpoint over tracked fixtures; excludes read-to-edit crossing, same-name resolution, wrapper aliases, parenthesized BindingIds, private source results, and complete exit cleanup; no Text bridge | `task bounded-bytes` | `bounded-bytes` |
