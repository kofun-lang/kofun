...
| A Text into a Bytes carrier, and a checked range out as Text (#1322) | `emit_bytes_mutation_call` lowers `assign_text` and `text` like the rest of the family; the emitted `stage2_bytes_assign_text` writes exact UTF-8 bytes transactionally through `kofun_bytes_grow`, and `stage2_bytes_text` runs `stage2_bytes_text_check` (range, then the 255-byte limit, then one UTF-8 scan) and returns a Text slot or raises `R029`..`R032` with an empty result | `task bytes-text` |
