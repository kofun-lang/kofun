...
| sustained fuzzing without unresolved critical findings | partial | a scheduled lane runs nine of the ten `task fuzz` generators daily with a seed that changes per run, and `tests/fuzz/FINDINGS.md` records what they find, so this is now **measurable** rather than unmeasurable — it is met only once the lane has run without an `open` row. `task fuzz` inside `verify` still runs from committed seeds on purpose, so a red pull request means the change broke something. Still absent: a corpus that grows and coverage instrumentation | `grep -rlE "schedule:" .github/workflows/` → `fuzz.yml`; `ls tests/fuzz/FINDINGS.md` → present; `grep -c FUZZ_SEED= .github/workflows/fuzz.yml` → 9 |
