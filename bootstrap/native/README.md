...
`LC_LOAD_DYLINKER`, an empty `LC_DYLD_INFO_ONLY` contract, `/usr/lib/libSystem.B.dylib`
in `LC_LOAD_DYLIB`, a macOS 11 `LC_BUILD_VERSION`, and `LC_MAIN`. The dyld-info
command declares that the bounded image has no rebases, binds, or exports and
prevents the classic-relocation fallback. The system-library declaration lets dyld
